...
class BloodBankNetworkingController extends Controller
{
    ...
    function proceedRelease(Request $r){
        $verifier = $r->get('verifier');
        $user = $r->get('user');
        $units = $r->get('units');
        $intent_id = $r->get('intent_id');

        $intent = NetworkIntent::find($intent_id);
        $intent->released_by = $user;
        $intent->released_dt = date('Y-m-d H:i:s');
        $intent->verified_by = $verifier['username'];
        $intent->save();

        foreach($units as $u){
            if($u['cancel']){
                DB::statement(
                    DB::raw("UPDATE component 
                    SET comp_stat = 'AVA', 
                    location = '".$intent->from."' 
                    WHERE donation_id = '".$u['donation_id']."' 
                    AND component_cd = '".$u['component_cd']."'"));
            }else{
